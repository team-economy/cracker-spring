...
import com.cracker.cracker.auth.dto.KakaoUserInfo;
...
//        }
    ...
    public TokenDto kakaoJoin(KakaoUserInfo userInfo, HttpServletResponse response) {
        ...
        String adminToken = "";
        JoinDto joinDto = new JoinDto(userInfo.getEmail(), password, userInfo.getNickname(), pic, marker_pic, role, adminToken);
    ...
    public TokenDto kakao(String authorizedCode, HttpServletResponse response) {
        ...
        Optional<Users> user = userRepository.findByEmail(userInfo.getEmail());
        TokenDto token;
        if (user.isPresent()) {
            // 이미 존재하는 회원이면 토큰 발급
